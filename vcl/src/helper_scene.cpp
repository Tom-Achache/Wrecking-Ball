...
void load_shaders(std::map<std::string,GLuint>& shaders)
{
    ...
    shaders["wireframe"] = create_shader_program("../../shaders/wireframe/wireframe.vert.glsl","../../shaders/wireframe/wireframe.geom.glsl","../../shaders/wireframe/wireframe.frag.glsl");
    shaders["curve"] = create_shader_program("../../shaders/curve/curve.vert.glsl","../../shaders/curve/curve.frag.glsl");
...
void setup_scene(scene_structure &scene, gui_structure& gui)
{
    ...
    scene.camera.perspective = perspective_structure( float(40*M_PI/180), aspect_ratio, 0.01f, 500.0f);
...
void gui_start_basic_structure(gui_structure& gui, scene_structure& scene, const std::map<std::string,GLuint>& shaders)
{
    ...
    if(ImGui::Button("Frame camera"))
        gui.show_frame_camera = !gui.show_frame_camera;
    if(ImGui::Button("Frame worldspace"))
        gui.show_frame_worldspace = !gui.show_frame_worldspace;
