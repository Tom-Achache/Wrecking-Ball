...
std::vector<index3> connectivity_grid_w_holes(size_t Nu, size_t Nv, std::set<int> deleted)
{
    std::vector<std::array<unsigned int,3> > triangle_index;
    for(size_t ku=0; ku<Nu-1; ++ku) {
        for( size_t kv=0; kv<Nv-1; ++kv) {
            
            if(deleted.count(kv + Nv*ku) < 1){
            const unsigned int u00 = static_cast<unsigned int>( kv + Nv*ku );
            const unsigned int u10 = static_cast<unsigned int>( kv+1 + Nv*ku );
            const unsigned int u01 = static_cast<unsigned int>( kv + Nv*(ku+1) );
            const unsigned int u11 = static_cast<unsigned int>( kv+1 + Nv*(ku+1) );
...
mesh mesh_primitive_sphere(float radius, const vec3& p0, size_t Nu, size_t Nv)
{
    ...
    for( size_t ku=0; ku<Nu; ++ku ) {
        for( size_t kv=0; kv<Nv; ++kv ) {
            ...
            const float theta = static_cast<float>( M_PI*u );
            const float phi   = static_cast<float>( 2*M_PI*v );
...
mesh mesh_primitive_cylinder(float radius,  const vec3& p1, const vec3& p2, size_t Nu, size_t Nv)
{
    ...
    for( size_t ku=0; ku<Nu; ++ku ) {
        for( size_t kv=0; kv<Nv; ++kv ) {
            ...
            const float theta = static_cast<float>( 2* M_PI * u );
...
mesh mesh_primitive_quad(const vec3& p00, const vec3& p10, const vec3& p01)
{
    // Check that quadrangle is not degenerated (vertices with same geometric positions)
    assert(norm(p10-p00)>1e-6f);
    assert(norm(p01-p00)>1e-6f);
    assert(norm(p10-p01)>1e-6f);

    // Compute fourth vertex position
    const vec3 p11 = p10 + p01 - p00;
    ...
    quad.texture_uv   = {{0,1}, {1,1}, {1,0}, {0,0}}; // 2D (u,v) - texture coordinates
...
mesh mesh_primitive_disc(float radius, const vec3& p0, const vec3& n, size_t N)
{
    ...
    for(size_t k=0; k<N; ++k)
    {
        ...
        const float theta = 2*static_cast<float>(M_PI)*u;
