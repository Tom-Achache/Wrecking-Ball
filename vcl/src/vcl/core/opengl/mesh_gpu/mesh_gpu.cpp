...
void mesh_gpu::update_connectivity(const std::vector<index3>& new_connectivity){
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo_index);
    assert(glIsBuffer(vbo_index));
    
    glBufferSubData(GL_ELEMENT_ARRAY_BUFFER,0, GLsizeiptr(new_connectivity.size()*sizeof(GLuint)*3), &new_connectivity[0]);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
    }
